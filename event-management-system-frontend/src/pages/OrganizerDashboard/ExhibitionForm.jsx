              ...
              >
                ...
                {/* Stall Allocation Section */} 
                  <div className="p-4 border border-sky-300 rounded-lg">
                    ...
                    {/* Upload Place Map (if not already moved here) */}
                    <div className="mb-4">
                      ...
                      onClick={async () => {
                        try {
                          const res = await fetch(
                            `http://localhost:8000/allocate-stalls?total_stalls=${form.stalls}`
                          );
                          const blob = await res.blob();
                          const imageUrl = URL.createObjectURL(blob);
                          setStallMapPreview(imageUrl);
                          setForm((prev) => ({
                            ...prev,
                            placeMap: true, // We use this flag to show the image
                          }));
                          // setStallMapPreview(imageUrl);
                        } catch (error) {
                          console.error("Error generating stall allocation:", error);
                          alert("Something went wrong while generating the stall layout.");
                        }
                      }}
