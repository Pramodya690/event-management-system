...
app.post('/api/auth/attendee/signup', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO attendee (name, email, gender,age,city,password) VALUES ($1, $2, $3,$4,$5,$6) RETURNING *',
  ...
  try {
    const result = await pool.query(
      `INSERT INTO tickets 
       ...
       RETURNING *`,
      [event_id, type, name, quantity, price || 0, sales_start, sales_end]
    );
    console.log("shaw",event_id)
...
// in your Express app, next to saveLayout…
app.post(
  '/api/saveStallLayout/:eventId',
  upload.single('stallLayoutImage'),
  async (req, res) => {
    console.log('params:', req.params);
    console.log('file received:', !!req.file);
    if (!req.file) {
      return res.status(400).json({ error: 'No file received—check your form-data key!' });
    }
    ...
    try {
      const result = await pool.query(
        `UPDATE event
         SET stall_layout_image = $1
         WHERE id = $2
         RETURNING id, stall_layout_image IS NOT NULL AS saved`,
        [img, id]
      );
      console.log('DB result:', result.rows);
      if (!result.rows.length) return res.status(404).json({ error: 'Event not found' });
      if (!result.rows[0].saved) return res.status(500).json({ error: 'Column stayed NULL' });
      res.json({ message: 'Stall layout saved!', event: result.rows[0] });
    } catch (err) {
      console.error('saveStallLayout error:', err);
      res.status(500).json({ error: err.message });
    }
  }
);
