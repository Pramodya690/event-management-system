...
const OrganizerDashboardSidebar = () => {
  ...
  const isCreateEventRoute = location.pathname.startsWith(
  ...
  return (
    <div className="flex bg-sky-100 text-gray-800">
      {/* Fixed Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen bg-white shadow-lg transition-all duration-300 z-30 ${
          expanded ? "w-64" : "w-20"
        }`}
      >
        {/* Sidebar Header */}
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          {expanded ? (
            <NavLink
              to="/"
              className="text-2xl font-bold text-sky-600 tracking-tight hover:underline"
            >
              EventSphere
            </NavLink>
          ) : (
            ...
            {expanded && isCreateEventRoute && (
              <div className="ml-8 mt-1 flex flex-col gap-1">
                <SubNavItem
                  ...
                  active={
                    location.pathname ===
                    "/organizer-dashboard/create/conference"
                  }
                >
                  Build Event Page
                </SubNavItem>
      ...
      <main
        ...
        } flex-grow h-screen overflow-y-auto bg-gray-50`}
      >
        {/* Top Navigation Bar */}
